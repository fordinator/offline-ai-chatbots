...
VOICE = "AZURE"
# Valid values are: ELEVENLABS, PLAYHT, GVOICE, AZURE
...
Recommended female kokoro Voice ID -- af_heart
...
DYNAMIC_GREETING_TEXT = "If you were a human, what would you look like?"
...
def read_history():
    ...
    conversation_list = ""
    return chat_history, conversation_list
...
#        console.print("[red]Error. Check your LLM type variable") 
    ...
    conversation_history.append({"content": summary})
                
    return conversation_history
    
def read_memories(conversation_history, chat_history):
    
    if MEMORIES_LLM in ["OPENAI", "MISTRAL"]:      
        if MEMORIES:
            if os.path.exists(MEMORIES_FILE):
                conversation_history = [{"role": "system", "content": system_message}]
                initial_summary = parse_memories(MEMORIES_FILE)
                conversation_history.append({initial_summary})
            else:
                conversation_history = [{"role": "system", "content": system_message}]
        else:
            conversation_history = [{"role": "system", "content": system_message}]
...
def speak_dynamic_greeting(greeting: str, conversation_list: str) -> str:
...
def speak_initial_greeting(greeting: str, conversation_list: str) -> str:

    if PRIMARY_LLM in ["OPENAI", "ANTHROPIC", "MISTRAL"]:
        chat_history.append({"role": "assistant", "content": greeting})
        if MEMORIES:
            conversation_list.append("Assistant:\n" + greeting + "\n\n")
    elif PRIMARY_LLM == "GOOGLE":
        chat_history.append({
            "role": "assistant",
            "parts": [{"text": greeting}]
...
if __name__ == "__main__":
    ...
    chat_history, conversation_list = read_history()   
    
    if DYNAMIC_GREETING: 
        speak_dynamic_greeting(DYNAMIC_GREETING_TEXT, conversation_list)
        
    if INITIAL_GREETING:    
        speak_initial_greeting(INITIAL_GREETING_TEXT, conversation_list)
