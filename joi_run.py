...
CHESS_MODE  = False                   # True while a game is running
CHESS_BOARD = None                    # python-chess Board instance
CHESS_LAST_SCORE = 0.0                # centipawns from Stockfish eval
...
# ────────────────────────────────────────────────────────────────────────
    ...
    class ChessGame:
        API_URL = "https://chess-api.com/v1"          # <— new endpoint

        @staticmethod
        def _query_engine(fen: str, depth: int = 15):
            """Return (uci_move:str, score_cp:float, captured:bool)."""
            r = requests.post(ChessGame.API_URL,
                            json={"fen": fen, "depth": depth},
                            timeout=12)
            r.raise_for_status()
            data = r.json()

            # accept either streaming ("move") or final ("bestmove") key
            uci = data.get("move") or data.get("bestmove")
            if not uci:
                raise RuntimeError(f"API reply missing move field: {data}")

            score = float(data.get("eval") or data.get("centipawns", 0))
            captured = bool(data.get("captured"))
            return uci, score, captured

        @staticmethod
        def _ascii(board: chess.Board) -> str:
            """Return an ASCII board with letters for the pieces."""
            symbols = {
            ...
            for rank in range(8, 0, -1):
                ...
                for file in range(1, 9):
                    ...
                    if piece:
                        ...
                        row.append(char.upper() if piece.color else char.lower())
                    else:
                        row.append(".")
        ...
        # -----------------------------------------------------------------
        @staticmethod
        def _query_engine(fen: str, depth: int = 15) -> Tuple[str, float]:
            """
            POST current FEN, get best move plus evaluation (centipawns).
            Returns (uci_move, score_cp).
            """
            payload = {"fen": fen, "depth": depth}
            r = requests.post(Games.ChessGame.API_URL, json=payload, timeout=10)
            r.raise_for_status()
            data = r.json()
            # API returns {"bestMove": "e2e4", "evaluation": 23}
            return data["bestMove"], float(data.get("evaluation", 0))
    
        # -----------------------------------------------------------------
        @staticmethod
        def start():
            global CHESS_MODE, CHESS_BOARD, CHESS_LAST_SCORE
            CHESS_MODE, CHESS_BOARD = True, chess.Board()
            CHESS_LAST_SCORE = 0.0
            console.print("[cyan]New chess game started — you’re White.")
            console.print(ChessGame._ascii(CHESS_BOARD))hite.")
            console.print(Games.ChessGame._ascii(CHESS_BOARD))
    
        # -----------------------------------------------------------------
        @staticmethod
        def handle_user_move(move_txt: str):
            """
            Validate the user’s SAN/lan/uci move, update board, compliment,
            then get Stockfish’s reply and maybe insult.
            """
            global CHESS_MODE, CHESS_BOARD, CHESS_LAST_SCORE
             
            captured = False
            try:
                # Accept either SAN (“Nf3”) or UCI (“g1f3”)
                move = CHESS_BOARD.parse_san(move_txt) if not re.match(r"^[a-h][1-8][a-h][1-8]", move_txt) \
                    else chess.Move.from_uci(move_txt)
            except ValueError:
                speak_greeting("That’s not a legal move, genius. Try again.", False, chat_history)
                return
    
            # ── replace the old compliment block ─────────────────────────────
            if captured:                       # you just took one of theirs
                speak_greeting(
                    "Give me a one-line witty compliment about that chess move.",
                    True,                      # <-- dynamic!
                    chat_history,
                )
            else:
                speak_greeting(                # neutral encouragement
                    "Give me a single-sentence friendly response to a solid move.",
                    True,
                    chat_history,
                )   
            ...
            fen = CHESS_BOARD.fen()
            try:
                ai_move_uci, score = Games.ChessGame._query_engine(fen)
                ai_move = chess.Move.from_uci(ai_move_uci)
            except Exception as e:
                console.print(f"[red]Chess-API failed: {e}")
                speak_greeting("Engine went on strike. Game over.", False, chat_history)
                CHESS_MODE = False
                return
    
            CHESS_BOARD.push(ai_move)
                
            uci, score, ai_captured = ChessGame._query_engine(fen)
                  
            console.print("[magenta]Engine plays", ai_move_uci)
            console.print(Games.ChessGame._ascii(CHESS_BOARD))
            # Insult if the engine captured
            if ai_captured:
                speak_greeting(
                "Give me a short, playful insult because you just captured a piece.",
                True, chat_history)
            elif score - CHESS_LAST_SCORE < -50:
                speak_greeting(
                "Give me a single-sentence taunt about opponent blundering.",
                True, chat_history)
            else:
                speak_greeting(
                "Your move again. Respond in one friendly sentence.",
                True, chat_history)
                CHESS_LAST_SCORE = score
        
            # Game over?
            if CHESS_BOARD.is_game_over():
                res = CHESS_BOARD.result()
                speak_greeting(f"Game over. Result: {res}", False, chat_history)
                CHESS_MODE = False
...
def process_user_text(user_text: str, input_mode: str):
    ...
    # ─── CASE: Start a chess game ───────────────────────────────────
    if lower_text.startswith("let's play chess"):
        append_to_history("user", user_text)
        Games.ChessGame.start()
        return

    # ─── While in chess mode, every line is treated as a move ──────
    if CHESS_MODE:
        append_to_history("user", user_text)
        Games.ChessGame.handle_user_move(user_text.strip())
        return
