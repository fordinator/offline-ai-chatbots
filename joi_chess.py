...
PRIMARY_LLM = "COPILOT"
MEMORIES_LLM = ""
# Valid values are: OPENAI, GOOGLE, COPILOT, ANTHROPIC, MISTRAL, GROQ, OLLAMA
...
OPENAI_API_KEY = "s"
GOOGLE_API_KEY = ""
COPILOT_API_ENDPOINT = "https://cacophony4.openai.azure.com/"
COPILOT_API_VERSION = "2024-10-21"
COPILOT_DEPLOYMENT_NAME = "cacobot-gpt-4o"
COPILOT_API_KEY1 = ""
COPILOT_TENANT_ID = ""
COPILOT_CLIENT_ID = ""
...
ENGINE_DEPTH = 4               # 1-30 → higher = stronger/slower
...
with console.status("[yellow]→ Importing libraries… Please wait… [red]25% COMPLETE",spinner="dots"):
    ...
    import json
with open("iching.json", "r", encoding="utf-8") as f:
...
RIMSHOT_FILES = [                     # Played after each joke
    "joke-drums-2-259684.mp3",
    "joke-rim-shot-276687.mp3",
    "joke-drums-242242.mp3",
]
DIRTY = False
...
# ────────────────────────────────────────────────────────────────────────
            ...
            q0 = f"Question {TRIVIA_INDEX+1} of 3: " + TRIVIA_QUESTIONS[0]["question"]
    ...
    class ChessSession:
        ...
        def start_chess(self):
            self.llm("You and I are beginning a game of chess. Address me with an insulting taunt about how I am about to lose badly.")
        ...
        def ai_reply(self):
            ...
            if self.board.is_game_over():
                ...
                self.speak(f"Checkmate! {res}")   

                # If White (human) won:
                if result == "0-1":
                    # One final insulting rant:
                    self.llm("Address me with a multi-sentence ranty insult about how you are a superior chess player "
                            "because you just won the game!")
                if result == "1-0":
                    # A gracious compliment:
                    self.llm("Address me with a sincere one- or two-sentence compliment about how I played well "
                            "and you deserved that win.")
        
                # If it's a draw (optional):
                else:  # result == "1/2-1/2"
                    self.speak("It's a draw. Well played!")
        
                # Always say “Good game” aloud:
...
class Humor:
    ...
    def _fetch_jokes() -> None:
        ...
        if os.path.exists(JOKES_CACHE_FILENAME):
            try:
                with open(JOKES_CACHE_FILENAME, "r", encoding="utf-8") as f:
                    data = json.load(f)

                # Validate that it’s a list of ≥100 nonempty strings
                if (
                    ...
                    JOKES_CACHE.extend(item.strip() for item in data if item.strip())
                    return  # Done—cache is loaded from file!

            except Exception:
                # Something was wrong with the file (corrupted, partial, etc.)
                # Fall through to re‐fetch below
                pass
                
        if os.path.exists(DIRTY_JOKES_CACHE_FILENAME):
            try:
                with open(DIRTY_JOKES_CACHE_FILENAME, "r", encoding="utf-8") as f:
                    data = json.load(f)
        ...
        if DIRTY:
            if DIRTY_JOKES_CACHE:
                return
            system_msg = ( f"""
                You are a dirty joke generator. I want you to parse
                https://night.channel/75-dirty-jokes-that-are-never-appropriate-but-always-funny/
                and index all the jokes. Return **ONLY** a valid JSON **object** with a single 
                property `"jokes"`. That property must be an array of items, each an object
                containing **"question"** and **"answer"** keys.
            ...
            if JOKES_CACHE:
                return
            system_msg = ( f"""
                You are a joke generator. I want you to parse
        ...
        # ─── 2a) Write the newly fetched list back to disk ─────────
        try:
            with open(JOKES_CACHE_FILENAME, "w", encoding="utf-8") as f:
                json.dump(JOKES_CACHE, f, ensure_ascii=False, indent=2)
            with open(DIRTY_JOKES_CACHE_FILENAME, "w", encoding="utf-8") as g:
                json.dump(DIRTY_JOKES_CACHE, g, ensure_ascii=False, indent=2)

        except Exception as disk_err:
            console.print(f"[red]Warning: could not write jokes_cache.json: {disk_err}")
    ...
    def tell_random_joke() -> str:
        ...
        try:
            ...
            console.print(f"[red]Failed to get random number: {e}")
        try:
...
class WakeWordListener(threading.Thread):
    ...
    def handle_wake_word(self):
        ...
        while True: 
            console.print("[green]Now recording on default device…")

            user_text = get_voice_input()                
            
            if not user_text or user_text.strip() == "":
                speak_greeting("I'm going back to whatever it was I was doing now.", False, chat_history)
                console.print("[red]No audio detected; returning to wake-word listening.")
                return

                # 3) Run the normal “text → LLM → TTS → audio” pipeline:
               
            process_user_text(user_text, "voice")
...
class ChatHistory:
    ...
    def read_history(self):
        ...
        defaults = {
            ...
            "COPILOT":  [{"role": "system", "content": system_message}],
...
class SpeechToTextService:
    ...
    def record_voice(self, stop_event: threading.Event, q: Queue) -> None:
        ...
        silence_threshold = 0.01    # RMS < 0.01 ⇒ “quiet”
        silence_timeout = 3.0       # stop after 2 s of continuous quiet
        recorded_chunks = []
        last_loud_time = time.time()
        stream_error = None

        def callback(indata, frames, time_info, status):
            nonlocal last_loud_time, stream_error
            if MIC_MUTED:
                return
            else:
                try:    
                    ...
                    recorded_chunks.append(chunk_copy)
    
                    # Compute RMS
                    # `indata` is an np.ndarray of shape (frames, channels)
                    audio_float = chunk_copy.astype(np.float32) / 32768.0
                    rms = np.sqrt(np.mean(audio_float**2))
    
                    # If above threshold, this is “loud”
                    if rms > silence_threshold:
                        last_loud_time = time.time()
                    # Else do nothing; keep the old last_loud_time
        ...
        try:
            with sd.InputStream(
                ...
                while True:
                    ...
                    elapsed_quiet = time.time() - last_loud_time
                    if elapsed_quiet >= silence_timeout:
                        break
            ...
            # Concatenate into one big numpy array of shape (sum_chunks, 1)
...
class TextToSpeechService:
    ...
    def synthesize_playht(self, text: str) -> bytes:
        ...
        for chunk in self._playht_client.tts(
...
class QueryLLMService:
    ...
    def __init__(self):
        ...
        self._copilot_client = AzureOpenAI(
            api_version=COPILOT_API_VERSION,
            azure_endpoint=COPILOT_API_ENDPOINT,
    ...
    def generate_copilot(self, chat_history: list) -> str:
        """
        Generates a chat response using the Azure OpenAI (Copilot) API
        with Microsoft Entra ID authentication.
        """
        if not self._copilot_client:
            return "Error: Copilot client is not initialized. Please check your Azure credentials and endpoint."
            
        try:
            response = self._copilot_client.chat.completions.create(
                model=COPILOT_DEPLOYMENT_NAME, # Important: Use the deployment name here
                messages=chat_history,
                # Temperature and max_tokens can be added here if needed
                # temperature=1.0,
                # max_tokens=2048,
            )
            return response.choices[0].message.content.strip()
            ...
            console.print(f"[red]Error during Copilot API call: {e}")
            # You might want to return a more user-friendly error message
            return f"An error occurred while contacting the Copilot service: {e}"
...
LLM_DISPATCH = {
    ...
    "COPILOT":  (lambda text, hist: llm.generate_copilot(hist),      "earth"),
...
def process_user_text(user_text: str, input_mode: str):
    ...
    if TRIVIA_MODE:
        ...
        speak_greeting(f"The answer was '{correct}.' Due to parsing issues, I'll allow you to determine if you were correct.", False, chat_history)

        TRIVIA_INDEX += 1
        if TRIVIA_INDEX < len(TRIVIA_QUESTIONS):
            next_q = TRIVIA_QUESTIONS[TRIVIA_INDEX]["question"]
            # Speak question 2 or 3:
            speak_greeting(f"Question {TRIVIA_INDEX+1} of 3: {next_q}", False, chat_history)
        else:
            speak_greeting("Trivia complete! I hope you had fun, man.", False, chat_history)
        ...
        return
    # AKINATOR
    trigger = "i summon the genie"
    
    if lower_text.startswith(trigger):
        append_to_history("user", user_text)
        if input_mode != "text":
            console.print(f"\n[yellow]You: {user_text}")
        Games.start_akinator_session()      # defaults to English, no child mode
        return
   ...
   # ───────────────────────────────
    ...
    if lower_text.startswith("let's play chess"):
        chess_session = Games.ChessSession(
            speak_fn=lambda t: speak_greeting(t, False, chat_history),
            llm_prompt_fn=lambda p: speak_greeting(p, True, chat_history),
            
        )
        
        chess_session.start_chess()
        
        return
        
    if chess_session:
        chess_session.user_move(user_text)
        if chess_session.board.is_game_over():
            chess_session = None
        return        
    ...
    try: 
        ...
        if KIOSK_MODE:
            while not SHUTDOWN_EVENT.is_set():
                ...
                console.print("[yellow bold]Joi is always listening… [cyan]Press Ctrl+C to exit.\n")
        ...
        while not SHUTDOWN_EVENT.is_set():
            if input_mode == "text":
                ...
                if user_text.lower() == "/voice":
                    ...
                    console.print("[yellow]Already in [red]TEXT[yellow] mode.")
                ...
                try:
                    ...
                    if user_text.strip.lower() == "switch to voice mode":
                        console.print("[yellow]Already in [red]VOICE[yellow] mode.")
                        continue
                
                    if user_text.strip.lower() == "switch to text mode":
                        input_mode="text"
                        console.print("[yellow]Switched to [red]TEXT[yellow] mode.")
                        continue
